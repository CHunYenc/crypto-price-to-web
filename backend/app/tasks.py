...
def get_binance_tickers():
    ...
    NAME = str.upper("binance")
    for i in data:
        exchange_symbol = f"{NAME}_{i}"
        r.set(exchange_symbol, json.dumps(data[i]))
...
def setup_periodic_tasks(sender, **kwargs):
    ...
    sender.add_periodic_task(5.0, get_binance_tickers, name='get-binance-every-5')
    sender.add_periodic_task(5.0, get_cryptocom_tickers, name='get-cryptocom-every-5')
